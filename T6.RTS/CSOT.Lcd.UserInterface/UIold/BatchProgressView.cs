...
{
    public partial class BatchProgressView : XtraPivotGridControlView
    {
        ...
        public BatchProgressView(IServiceProvider serviceProvider)
        ...
        private void SetControl()
        {
            ...
            foreach (DataRow pRow in prodTable.Rows)
            {
                ...
                if (prod.ProductGroup == "BARE" || prod.ProductGroup == "CELL")
                    continue;
        ...
        private bool IsShopStep(string stepID)
        {
            return stepID == "T00000" || stepID == "F00000" ;
        }
        ...
        private void BindGrid()
        {
            ...
            foreach (ProductInfo info in _resultDict.Values)
            {
                ...
                row[JobChangePlanData.BATCH_ID] = info.BatchID;
                ...
                row[JobChangePlanData.AVG_TAT] = info.CalculateAvgTat().ToString("0.0");
                ...
                row[JobChangePlanData.IN_DURATION] = (info.InEndTime - info.InTime).TotalHours.ToString("0.0");
                row[JobChangePlanData.OUT_DURATION] = (info.OutTime - info.OutStartTime).TotalHours.ToString("0.0");

                if ((info.OutTime - info.OutStartTime).Minutes < 0)
                    row[JobChangePlanData.OUT_DURATION] = 0;
        ...
        private DataTable CreateDataTable()
        {
            ...
            dt.Columns.Add(JobChangePlanData.BATCH_ID, typeof(string));
        ...
        private void gridView1_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            ...
            if (rowIndex < 0)
            {
                //XtraPivotGridHelper.ClearPivotGridField;
                return;
            }

            DataRow row = (gridControl1.DataSource as DataTable).Rows[rowIndex];

            string batchID = row[JobChangePlanData.BATCH_ID].ToString();

            this.dockPanel1.Text = "BATCH_ID : " + batchID;

            BindPivotGrid(batchID);
        ...
        internal class InOutTime
        {
            ...
            public float GetTat()
            {
                if (InTime == DateTime.MinValue || OutTime == DateTime.MinValue)
                    return 0.0f;
                return (float)(OutTime - InTime).TotalDays;
            }
        }
        ...
        internal class ProductInfo
        {
            ...
            public DateTime InTime;
            public DateTime OutTime;
            public DateTime InEndTime;
            public DateTime OutStartTime;

            public float InDuration;
            public float OutDuration;

            public Dictionary<string, InOutTime> TimeDict;

            public ProductInfo(string prodID, string batchID, string shopID)
            {
                ...
                this.InEndTime = DateTime.MinValue;
                this.OutStartTime = DateTime.MaxValue;

                this.InDuration = 0.0f;
                this.OutDuration = 0.0f;

                TimeDict = new Dictionary<string, InOutTime>();
            }
            ...
            public void SetInOutTime(DateTime inTime, DateTime outTime, bool IsInStep)
            {
                if (IsInStep)
                {
                    if(this.InTime > inTime)
                        this.InTime = inTime;
                    if (outTime > this.InEndTime)
                        this.InEndTime = outTime;
                }
                ...
                if (!IsInStep && this.OutTime < outTime)
                {
                    ...
                    if (inTime < this.OutStartTime)
                        this.OutStartTime = inTime;
                }
            }

            public float CalculateAvgTat()
            {
                float count = 0;
                float totTAT = 0.0f;

                foreach (InOutTime item in this.TimeDict.Values)
                {
                    if (item.GetTat() > 0.0f)
                    {
                        count += 1.0f;
            ...
            internal void LotOutTime(string lotID, DateTime outTime)
            {
                InOutTime item;
                if (this.TimeDict.TryGetValue(lotID, out item) == false)
                {
                    item = new InOutTime();
                    this.TimeDict.Add(lotID, item);
                }
                item.OutTime = outTime;
            }

            internal void LotInTime(string lotID, DateTime inTime)
            {
                InOutTime item;
                if (this.TimeDict.TryGetValue(lotID, out item) == false)
                {
        ...
        internal class DetailInfo
        {
            ...
            public DetailInfo(string batchID, string shopID, string layer, string stepID, string stepDesc)
            {
                this.BatchID = batchID;
                this.ShopID = shopID;
                this.StepID = stepID;
            ...
            internal float GetSpan()
            {
                if (LastTime == DateTime.MinValue)
                    return 0.0f;

                return (float)(LastTime - FirstTime).TotalHours;
            }
        }
