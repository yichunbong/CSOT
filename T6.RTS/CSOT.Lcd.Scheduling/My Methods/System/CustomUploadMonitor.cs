...
{
    [FeatureBind()]
    public class CustomUploadMonitor : UploadMonitor
    {
        IModelLog _log;
        ...
        public CustomUploadMonitor(IModelLog log, ICollection<string> criticals = null)
            : base(log, criticals)
        {
            _log = log;
            if (criticals != null)
        ...
        protected override void OnException(string name, Exception ex)
        {
            ...
            if (_log != null)
            {
                ...
                _log.MonitorInfo("Upload '" + name + "' table error.\r\n" + ex.ToString());

                Outputs.DBError info = new DBError();
                info.TABLE = name;
                info.ERROR_LOG = string.Format("Upload '{0}' talble error.\r\n {1}", name, ex.ToString());
